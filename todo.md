...
- [x] 타이머 닫기 버튼을 왼쪽 위로 (`WindowCloseButton`, TimerView.swift)
  - macOS 27 창 닫기 버튼과 같은 14pt 빨간 원, 버튼 호버 시 ×, 누르면 살짝 어두워짐.
  - 링 출발점(좌상단 대각선)에 올린다 — 200pt 에서 (15.5, 15.5) 로 macOS 닫기 버튼 중심 (16, 16) 과 같다.
  - 위젯 창에도 추가: 닫기 = 감추기 (패널 "보이기"로 복귀). "윈도우" 창은 창을 닫는다.
  - 확인: 위젯·윈도우 호버 시 표시, 클릭으로 닫힘/감춤, 버튼에서 시작한 끌기는 창을 옮기지 않음,
    그 옆 몸통 끌기는 1:1 이동, L(420pt) 크기에서 위치.
- [ ] 다크 · 카멜레온 테마에서 닫기 버튼 테두리(배경색 halo) 눈으로 확인
